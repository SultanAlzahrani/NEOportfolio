  ...
  onMount(() => {
    const get = async () => {
      const request = await fetch(
        "https://api.sultanonline.app/heartbeat/status",
      );
...
<section class="side">
  ...
  <div class="flex justify-center items-center absolute -top-2 right-2 -z-10">
    <div
      class="size-4 rounded-full {computerData.data?.online
        ? 'bg-green'
        : 'bg-red'}"
    ></div>
    <div
      class="size-4 rounded-full absolute animate-duration-2s animate-delay-2s {computerData
        .data?.online
  ...
  <span>I'M</span>
  <span
    class={`font-black ${computerData.data?.online ? "text-green" : "text-red"}`}
    >{computerData.data?.online ? "ONLINE" : "OFFLINE"}.</span
  >
